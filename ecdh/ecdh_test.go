...
type validateKeyTestPair struct {
	keys [][]byte
	res  bool
}
...
var validateKeyPairTests = []validateKeyTestPair{
	{
		[][]byte{
...
func TestKeyPair(t *testing.T) {
	...
	for _, pair := range validateKeyPairTests {
		output := ecdh.ValidateKeypair(pair.keys[0], pair.keys[1])
		if output != pair.res {
			t.Error(
				"For", pair.keys,
				"expected", pair.res,
